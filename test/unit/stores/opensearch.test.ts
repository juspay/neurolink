/**
 * OpenSearchStore Unit Tests
 *
 * Tests for the OpenSearch vector store implementation.
 *
 * Note: Due to the OpenSearchStore using dynamic imports for the OpenSearch client,
 * these tests focus on configuration and behavior that can be tested without
 * requiring the actual OpenSearch connection. Integration tests with a real
 * OpenSearch instance should be used for full API testing.
 */
...
import {
  ...
  OpenSearchStore,
} from "../../../src/lib/stores/database/opensearch.js";
...
describe("OpenSearchStore", () => {
  describe("constructor", () => {
    it("should create store with required config", () => {
      ...
      expect(store.getStoreName()).toBe("opensearch");
    });
    ...
    it("should accept k-NN engine config", () => {
      const store = new OpenSearchStore({
        node: "http://localhost:9200",
        knnEngine: "faiss",
      });
  ...
  describe("configuration types", () => {
    it("should accept minimal config", () => {
      ...
      const store = new OpenSearchStore(config);
      expect(store).toBeDefined();
    });

    it("should accept full config", () => {
      const config: OpenSearchConfig = {
        node: "https://my-cluster.us-east-1.es.amazonaws.com",
        username: "admin",
        password: "admin",
        awsRegion: "us-east-1",
        awsAccessKeyId: "AKIAIOSFODNN7EXAMPLE",
    ...
    it("should support nmslib engine", () => {
      const config: OpenSearchConfig = {
        node: "http://localhost:9200",
        knnEngine: "nmslib",
      };
    ...
    it("should support faiss engine", () => {
      const config: OpenSearchConfig = {
        node: "http://localhost:9200",
        knnEngine: "faiss",
      };
    ...
    it("should support lucene engine", () => {
      const config: OpenSearchConfig = {
        node: "http://localhost:9200",
        knnEngine: "lucene",
      };
  ...
  describe("store identification", () => {
    it("should return opensearch as store name", () => {
      const store = new OpenSearchStore({
        node: "http://localhost:9200",
      });
  ...
  describe("connect error handling", () => {
    it("should handle missing OpenSearch SDK gracefully", async () => {
      const store = new OpenSearchStore({
        node: "http://localhost:9200",
      });
