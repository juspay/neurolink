...
function stubNeuroLink(opts: {
  ...
  // Mirrors ToolsManager.getCustomTools()'s real return type
  // (`Map<string, unknown> | undefined`): an instance that never
  // initialized its custom-tools map returns undefined, not an empty Map.
  customToolsUndefined?: boolean;
}): NeuroLink {
  ...
  return {
    ...
    getCustomTools: (): Map<string, unknown> | undefined =>
      opts.customToolsUndefined ? undefined : customTools,
...
await test("getCustomTools() returning undefined does not throw — truncation still proceeds without naming retrieve_context (Yama MINOR on #1622)", async () => {
  const stub = stubNeuroLink({ customToolsUndefined: true });
  const result = (await runProbe(
    async () => ({ content: [textItem(BIG_TEXT)], isError: false }),
    stub,
  )) as Envelope;
  ...
  assert(
    ...
    "text should still be truncated when custom tools map is undefined",
  );
  assert(
    !text.includes(RETRIEVE_CONTEXT_TOOL_NAME),
    "notice must not name retrieve_context when the custom tools map is undefined",
  );
});
...
// The notice-suppression above is only worth anything if it reaches every
// truncation branch, not just the MCP-envelope one. These four drive the
// non-envelope shapes — a plain string, a string `content` field, a string
// `data` field, and the generic JSON sentinel — on an instance where
// retrieve_context is NOT registered. Each previously fell through to
// generateToolOutputPreview's built-in default, which names the tool
// unconditionally and so pointed the model at something it cannot call.

const nonEnvelopeShapes: Array<{
  what: string;
  make: () => unknown;
  read: (r: unknown) => string;
}> = [
  {
    what: "a plain oversized string result",
    make: () => BIG_TEXT,
    read: (r) => String(r),
  },
  {
    what: "an object with an oversized string `content`",
    make: () => ({ content: BIG_TEXT }),
    read: (r) => String((r as { content: unknown }).content),
  },
  {
    what: "an object with an oversized string `data`",
    make: () => ({ data: BIG_TEXT }),
    read: (r) => String((r as { data: unknown }).data),
  },
  {
    what: "an oversized plain object (generic JSON sentinel branch)",
    make: () => ({ rows: [BIG_TEXT], note: "not an envelope" }),
    read: (r) => JSON.stringify(r),
  },
];

for (const shape of nonEnvelopeShapes) {
  await test(`${shape.what}: notice omits retrieve_context when it is unregistered`, async () => {
    const stub = stubNeuroLink({});
    const result = await runProbe(async () => shape.make(), stub);
    const text = shape.read(result);

    // PRECONDITION: truncation must actually have happened, or "the notice
    // does not name the tool" is trivially true of an untouched payload.
    assert(
      Buffer.byteLength(text, "utf-8") < BIG_TEXT.length,
      "precondition: the result was not truncated at all",
    );
    assert(
      /bytes omitted/.test(text),
      "precondition: no omission notice was emitted",
    );
...
await test("a plain oversized string still names retrieve_context when it IS registered", async () => {
  const stub = stubNeuroLink({ registerRetrieveContext: true });
  const result = await runProbe(async () => BIG_TEXT, stub);
...
// `maxBytes` is a budget the caller relies on, and the notice is part of what
// gets returned. `headMaxBytes + tailMaxBytes` is exactly `maxBytes`, so
// appending the notice unmeasured put every truncated preview over budget —
// 82 bytes over for the default notice, and unboundedly over for a long
// custom one. These pin the guarantee across budgets and notice lengths.
for (const maxBytes of [500, 2_000, 8_000, 40_000]) {
  await test(`preview stays within maxBytes=${maxBytes} for default, short and long notices`, () => {
    const cases: Array<{ what: string; opts: Record<string, unknown> }> = [
      { what: "default notice", opts: { maxBytes } },
      {
        what: "short callback notice",
        opts: {
          maxBytes,
          notice: (n: number) => `\n\n[... ${n} bytes omitted ...]\n\n`,
        },
      ...
      {
        what: "300-byte literal notice",
        opts: { maxBytes, notice: "X".repeat(300) },
      },
    ];
    for (const c of cases) {
      ...
      // PRECONDITION: a preview that was never truncated is trivially
      // within budget and proves nothing about the notice.
      assert(truncated, `precondition: ${c.what} did not truncate at all`);
      const size = Buffer.byteLength(preview, "utf-8");
      assert(
        size <= maxBytes,
        `${c.what}: preview is ${size - maxBytes} bytes over the budget`,
      );
    }
  });
}

// The budget still has to hold when the notice ALONE is bigger than it. The
// reclaim loop can only take bytes back from `head` and `tail`; emptying both
// leaves the notice, so a caller asking for 100 bytes with a 300-byte notice
// was handed all 300. These budgets are smaller than the notices below on
// purpose — that is the whole case.
for (const maxBytes of [100, 40, 8]) {
  await test(`preview stays within maxBytes=${maxBytes} when the notice alone exceeds it`, () => {
    const cases: Array<{ what: string; opts: Record<string, unknown> }> = [
      {
        what: "300-byte literal notice",
        opts: { maxBytes, notice: "X".repeat(300) },
      ...
      {
        what: "300-byte callback notice",
        opts: { maxBytes, notice: () => "Y".repeat(300) },
      },
      { what: "default notice", opts: { maxBytes } },
      {
        // 3 bytes per character, so every one of these budgets lands
        // mid-sequence: a naive byte cut emits U+FFFD, which is itself 3
        // bytes and can put the result back over the budget.
        what: "600-byte multibyte notice",
        opts: { maxBytes, notice: "\u2192".repeat(200) },
      },
    ];
    for (const c of cases) {
      const { preview, truncated } = generateToolOutputPreview(
      ...
      assert(
        !preview.includes("\uFFFD"),
        `${c.what}: preview was cut mid-character`,
      );
    }
  });
}
