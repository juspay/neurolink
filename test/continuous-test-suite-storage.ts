...
const ADAPTER_CONFIGS = {
  ...
  postgresql: {
    type: "postgresql",
    connectionString: process.env.DATABASE_URL || "postgres://localhost:5432/neurolink_test",
...
async function runTest(name: string, testFn: () => Promise<boolean>): Promise<TestResult> {
  ...
  try {
    ...
    logTest(name, passed ? "PASS" : "FAIL", `Duration: ${formatDuration(duration)}`);
...
async function testMemoryStorageAdapter(): Promise<boolean> {
  const { MemoryAdapter } = await import("../src/lib/storage/adapters/memoryAdapter.js");
  ...
  try {
    ...
    const updatedRun = await adapter.updateWorkflowRunStatus(workflowRun.id, "completed", { result: "success" });
    ...
    if (stats.threadCount !== 1 || stats.messageCount !== 1) {
      log(`Stats verification failed: threads=${stats.threadCount}, messages=${stats.messageCount}`, "red");
      return false;
    }
...
async function testFileStorageAdapter(): Promise<boolean> {
  const { FileStorageAdapter } = await import("../src/lib/storage/adapters/FileStorageAdapter.js");
  ...
  try {
    ...
    if (messages.length !== 1 || messages[0].content !== "File storage test message") {
...
async function testSQLiteStorageAdapter(): Promise<boolean> {
  ...
  const { SQLiteStorageAdapter } = await import("../src/lib/storage/adapters/SQLiteStorageAdapter.js");

  const filename = path.join(os.tmpdir(), `neurolink-sqlite-test-${Date.now()}.db`);
  ...
  try {
    ...
    await adapter.setRecord("sqlite-ns", "ttl-key", { value: "expires" }, { ttl: 60 });
...
async function testRedisStorageAdapter(): Promise<boolean> {
  ...
  try {
    const { RedisAdapter } = await import("../src/lib/storage/adapters/redisAdapter.js");
    ...
    try {
      ...
      // TTL record
      await adapter.setRecord("redis-ns", "ttl-key", { data: "expires" }, { ttl: 10 });
...
async function testPostgreSQLStorageAdapter(): Promise<boolean> {
  ...
  try {
    const { PostgresAdapter } = await import("../src/lib/storage/adapters/postgresAdapter.js");
...
async function testMongoDBStorageAdapter(): Promise<boolean> {
  ...
  try {
    const { MongoDBAdapter } = await import("../src/lib/storage/adapters/mongodbAdapter.js");
...
async function testS3StorageAdapter(): Promise<boolean> {
  if (TEST_CONFIG.skipExternalDependencies || !process.env.AWS_ACCESS_KEY_ID) {
    log("Skipping S3 test (external dependencies disabled or no AWS credentials)", "yellow");
    return true;
  }

  try {
    const { S3StorageAdapter } = await import("../src/lib/storage/adapters/S3StorageAdapter.js");
...
async function testLibSQLStorageAdapter(): Promise<boolean> {
  try {
    const { LibSQLAdapter } = await import("../src/lib/storage/adapters/libsqlAdapter.js");

    const filename = path.join(os.tmpdir(), `neurolink-libsql-test-${Date.now()}.db`);
...
async function testCachingMiddleware(): Promise<boolean> {
  const { CachingMiddleware } = await import("../src/lib/storage/middleware/CachingMiddleware.js");
...
async function testEncryptionMiddleware(): Promise<boolean> {
  const { EncryptionMiddleware } = await import("../src/lib/storage/middleware/EncryptionMiddleware.js");
  ...
  try {
    ...
    if (typeof encrypted !== "object" || !(encrypted as Record<string, unknown>).__encrypted) {
...
async function testCompressionMiddleware(): Promise<boolean> {
  const { CompressionMiddleware } = await import("../src/lib/storage/middleware/CompressionMiddleware.js");
  ...
  try {
    ...
    if (typeof compressed === "object" && (compressed as Record<string, unknown>).__compressed) {
    ...
    if (typeof notCompressed === "object" && (notCompressed as Record<string, unknown>).__compressed) {
...
async function testMigrationRunner(): Promise<boolean> {
  const { MigrationRunner } = await import("../src/lib/storage/migrations/runner.js");
  const { MemoryAdapter } = await import("../src/lib/storage/adapters/memoryAdapter.js");
  ...
  try {
    ...
    if (initialStatus.pending.length !== 2) {
      log(`Initial pending count wrong: ${initialStatus.pending.length}`, "red");
      return false;
    }
    ...
    const v1_1AfterRollback = await storage.getRecord("migrations", "v1_1_applied");
...
async function testStorageFactory(): Promise<boolean> {
  const { StorageFactory, createStorage } = await import("../src/lib/storage/storageFactory.js");
  ...
  try {
    ...
    // Test createProvider
    const memoryProvider = await StorageFactory.createProvider("memory");
    ...
    const pgAlias = await StorageFactory.createProvider("postgres", {
...
async function testStorageRegistry(): Promise<boolean> {
  const { StorageRegistry, registerBackend, unregisterBackend, getBackend, getAvailableBackends } = await import(
    "../src/lib/storage/StorageRegistry.js"
  );
  ...
  try {
    ...
    if (!updatedBackends.includes("custom-test")) {
      log("Custom backend not registered", "red");
      return false;
    }

    // Get the backend
    const backend = await getBackend("custom-test");
    if (!backend) {
      log("Could not get custom backend", "red");
      return false;
    }
    ...
    if (!health.healthy) {
      log("Custom backend health check failed", "red");
      return false;
    }
    ...
    if (finalBackends.includes("custom-test")) {
      log("Custom backend not unregistered", "red");
      return false;
    }
...
async function testStorageWithMiddlewareStack(): Promise<boolean> {
  ...
  try {
    ...
    processed = await compression.beforeWrite("test-key", processed as { sensitiveInfo: string; content: string });
    processed = await encryption.beforeWrite("test-key", processed as { sensitiveInfo: string; content: string });
    processed = await caching.beforeWrite("test-key", processed as { sensitiveInfo: string; content: string });

    // Store in storage
    await storage.setRecord("middleware-test", "test-key", processed as { sensitiveInfo: string; content: string });
    ...
    retrieved = await caching.afterRead("test-key", retrieved as { sensitiveInfo: string; content: string });
    retrieved = await encryption.afterRead("test-key", retrieved as { sensitiveInfo: string; content: string });
    retrieved = await compression.afterRead("test-key", retrieved as { sensitiveInfo: string; content: string });
...
async function testWorkflowRunOperations(): Promise<boolean> {
  ...
  try {
    ...
    if (!currentRun?.stepResults || Object.keys(currentRun.stepResults).length !== 2) {
    ...
    if (!failedResult?.error || failedResult.error.code !== "VALIDATION_ERROR") {
...
async function runAllTests(): Promise<void> {
  ...
  adapterResults.push(await runTest("Memory Storage Adapter", testMemoryStorageAdapter));
  adapterResults.push(await runTest("File Storage Adapter", testFileStorageAdapter));
  adapterResults.push(await runTest("SQLite Storage Adapter", testSQLiteStorageAdapter));
  adapterResults.push(await runTest("LibSQL Storage Adapter", testLibSQLStorageAdapter));
  adapterResults.push(await runTest("Redis Storage Adapter", testRedisStorageAdapter));
  adapterResults.push(await runTest("PostgreSQL Storage Adapter", testPostgreSQLStorageAdapter));
  adapterResults.push(await runTest("MongoDB Storage Adapter", testMongoDBStorageAdapter));
  adapterResults.push(await runTest("S3 Storage Adapter", testS3StorageAdapter));
  ...
  suiteResults.push({
    ...
    skipped: adapterResults.filter((t) => t.details?.startsWith("Skipped")).length,
  ...
  middlewareResults.push(await runTest("Caching Middleware", testCachingMiddleware));
  middlewareResults.push(await runTest("Encryption Middleware", testEncryptionMiddleware));
  middlewareResults.push(await runTest("Compression Middleware", testCompressionMiddleware));
  ...
  integrationResults.push(await runTest("Storage with Middleware Stack", testStorageWithMiddlewareStack));
  integrationResults.push(await runTest("Thread and Message Lifecycle", testThreadMessageLifecycle));
  integrationResults.push(await runTest("Workflow Run Operations", testWorkflowRunOperations));
