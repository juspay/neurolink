...
describe("CompressionMiddleware", () => {
  ...
  describe("Compression with gzip", () => {
    ...
    it("should skip compression when result is larger", async () => {
      ...
      const randomData = { data: Buffer.from(Array(200).fill(0).map(() => Math.random())).toString("base64") };
  ...
  describe("Compression with deflate", () => {
    it("should compress and decompress with deflate", async () => {
      ...
      const decompressed = await deflateMiddleware.afterRead("key1", compressed);
      expect(decompressed).toEqual(original);
  ...
  describe("Compression Levels", () => {
    it("should compress more with higher level", async () => {
      ...
      const highCompressed = await highLevelMiddleware.beforeWrite("key1", data);

      // Higher level should produce smaller output (usually)
      expect((highCompressed as Record<string, unknown>).compressedSize).toBeLessThanOrEqual(
