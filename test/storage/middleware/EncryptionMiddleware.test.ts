...
describe("EncryptionMiddleware", () => {
  ...
  describe("AES-256-CBC Algorithm", () => {
    it("should encrypt and decrypt with CBC mode", async () => {
      ...
      expect((encrypted as Record<string, unknown>).algorithm).toBe("aes-256-cbc");
    ...
    it("should not include tag for CBC mode", async () => {
      ...
      const encrypted = await cbcMiddleware.beforeWrite("key1", { data: "test" });
  ...
  describe("ChaCha20-Poly1305 Algorithm", () => {
    it("should encrypt and decrypt with ChaCha20-Poly1305", async () => {
      ...
      expect((encrypted as Record<string, unknown>).algorithm).toBe("chacha20-poly1305");
  ...
  describe("Error Handling", () => {
    it("should throw on decryption with wrong key", async () => {
      ...
      const encrypted = await middleware1.beforeWrite("key1", { data: "secret" });
  ...
  describe("Security", () => {
    it("should zero out key on destroy", async () => {
      ...
      await expect(tempMiddleware.afterRead("key1", encrypted)).rejects.toThrow();
    });
  });
