...
describe("StorageFactory", () => {
  ...
  describe("Factory Creation", () => {
    ...
    it("should not cache when forceNew is true", async () => {
      const config: StorageConfig = { type: "memory" };
      const storage1 = await StorageFactory.createStorage(config);
      const storage2 = await StorageFactory.createStorage(config, { forceNew: true });
  ...
  describe("Environment Configuration", () => {
    ...
    it("should parse Redis URL from environment", () => {
      ...
      try {
        ...
        expect((config as { type: "redis"; url: string }).url).toBe("redis://localhost:6379");
      } finally {
        if (originalType) {
  ...
  describe("Cache Management", () => {
    ...
    it("should clear specific cache key", async () => {
      const config1: StorageConfig = { type: "memory" };
      const config2: StorageConfig = { type: "memory", maxSize: 100 };
