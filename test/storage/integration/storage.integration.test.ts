...
function createTestThread(overrides: Partial<CreateThreadInput> = {}): CreateThreadInput {
  return {
...
function createTestMessage(threadId: string, overrides: Partial<CreateMessageInput> = {}): CreateMessageInput {
  return {
    threadId,
    role: "user",
...
function createTestWorkflowRun(overrides: Partial<SaveWorkflowRunInput> = {}): SaveWorkflowRunInput {
  return {
...
describe("StorageFactory Integration", () => {
  ...
  describe("Provider Creation", () => {
    it("should create memory adapter via factory", async () => {
      const storage = await StorageFactory.createProvider("memory");
    ...
    it("should create memory adapter via alias", async () => {
      const storage = await StorageFactory.createProvider("mem");
    ...
    it("should throw for unknown backend type", async () => {
      ...
      await expect(
        StorageFactory.createProvider("unknown-backend")
    ...
    it("should support singleton instances with getOrCreate", async () => {
      ...
      const storage3 = await StorageFactory.getOrCreate("memory", undefined, "test-key");
...
describe("Memory Adapter CRUD Operations", () => {
  ...
  describe("Thread CRUD", () => {
    ...
    it("should filter threads by resourceId", async () => {
      await storage.createThread(createTestThread({ resourceId: "filter-user-1" }));
      await storage.createThread(createTestThread({ resourceId: "filter-user-1" }));
      await storage.createThread(createTestThread({ resourceId: "filter-user-2" }));

      const result = await storage.getThreadsByResourceId("filter-user-1");

      expect(result.data).toHaveLength(2);
      expect(result.data.every((t) => t.resourceId === "filter-user-1")).toBe(true);
    });
  ...
  describe("Message CRUD", () => {
    ...
    it("should create messages in batch", async () => {
      const inputs: CreateMessageInput[] = [
        ...
        createTestMessage(threadId, { role: "assistant", content: "Message 2" }),
    ...
    it("should list messages by thread with filtering", async () => {
      await storage.createMessage(createTestMessage(threadId, { role: "user" }));
      await storage.createMessage(createTestMessage(threadId, { role: "assistant" }));
      await storage.createMessage(createTestMessage(threadId, { role: "user" }));
  ...
  describe("Workflow Run CRUD", () => {
    ...
    it("should list workflow runs with filtering", async () => {
      ...
      await storage.saveWorkflowRun(createTestWorkflowRun({ workflowId, status: "completed" }));
      await storage.saveWorkflowRun(createTestWorkflowRun({ workflowId, status: "running" }));
      await storage.saveWorkflowRun(createTestWorkflowRun({ status: "pending" }));
    ...
    it("should update workflow run status", async () => {
      const run = await storage.saveWorkflowRun(createTestWorkflowRun({ status: "running" }));
  ...
  describe("Custom Record CRUD", () => {
    it("should set a custom record", async () => {
      const record = await storage.setRecord("test-ns", "key1", { data: "value" });
    ...
    it("should handle TTL expiration", async () => {
      ...
      await storage.setRecord("test-ns", "ttl-key", { data: "value" }, { ttl: 2 });
...
describe("Migration System", () => {
  ...
  describe("Migration Execution", () => {
      ...
      migrationRunner.registerAll([
        {
          version: "1.0.0",
          name: "step1",
          up: async () => { step1Ran = true; },
        },
        {
          version: "1.1.0",
          name: "step2",
          up: async () => { step2Ran = true; },
        },
      ]);
    ...
    it("should handle dry run mode", async () => {
      ...
      const dryRunMigrationRunner = createMigrationRunner(storage, { dryRun: true });
      dryRunMigrationRunner.register({
        version: "1.0.0",
        name: "test",
        up: async () => { actuallyRan = true; },
      });
...
describe("Connection Pool", () => {
  ...
  describe("Connection Acquisition", () => {
    ...
    it("should handle invalid connections", async () => {
      ...
      expect((newConn as MockConnection).id).not.toBe((conn as MockConnection).id);
...
describe("Batch Operations", () => {
  ...
  describe("Batch Message Creation", () => {
    it("should create 100 messages in batch", async () => {
      ...
      const inputs: CreateMessageInput[] = Array.from({ length: 100 }, (_, i) => ({
        threadId: thread.id,
    ...
    it("should maintain message order in batch", async () => {
      ...
      const inputs: CreateMessageInput[] = Array.from({ length: 10 }, (_, i) => ({
        threadId: thread.id,
        role: "user",
...
describe("Query Builder and Filtering", () => {
  ...
  describe("Thread Filtering", () => {
    beforeEach(async () => {
      // Create test data
      await storage.createThread(createTestThread({ resourceId: "user-a", status: "active", title: "Alpha" }));
      await storage.createThread(createTestThread({ resourceId: "user-a", status: "archived", title: "Beta" }));
      await storage.createThread(createTestThread({ resourceId: "user-b", status: "active", title: "Gamma" }));
      await storage.createThread(createTestThread({ resourceId: "user-b", status: "deleted", title: "Delta" }));
    });
  ...
  describe("Message Filtering", () => {
    ...
    beforeEach(async () => {
      ...
      await storage.createMessage(createTestMessage(threadId, { role: "user", content: "User 1" }));
      await storage.createMessage(createTestMessage(threadId, { role: "assistant", content: "Assistant 1" }));
      await storage.createMessage(createTestMessage(threadId, { role: "user", content: "User 2" }));
      await storage.createMessage(createTestMessage(threadId, { role: "system", content: "System 1" }));
    });
    ...
    it("should filter by message type", async () => {
      ...
      const result = await storage.listMessages({ threadId, type: "tool-result" });
  ...
  describe("Workflow Run Filtering", () => {
    beforeEach(async () => {
      await storage.saveWorkflowRun(createTestWorkflowRun({ workflowId: "wf-1", status: "completed" }));
      await storage.saveWorkflowRun(createTestWorkflowRun({ workflowId: "wf-1", status: "failed" }));
      await storage.saveWorkflowRun(createTestWorkflowRun({ workflowId: "wf-2", status: "running" }));
      await storage.saveWorkflowRun(createTestWorkflowRun({ workflowId: "wf-2", status: "completed", resourceId: "res-1" }));
    });
...
describe("Error Handling", () => {
  ...
  describe("Invalid Operations", () => {
    ...
    it("should return null when updating non-existent thread", async () => {
      const result = await storage.updateThread("non-existent-id", { title: "Updated" });
      expect(result).toBeNull();
    });
    ...
    it("should return false when updating step for non-existent run", async () => {
      const result = await storage.updateStepResult("non-existent-id", "step-1", {
        stepId: "step-1",
        status: "completed",
      });
      expect(result).toBe(false);
...
describe("Adapter Switching", () => {
  describe("Cross-Adapter Data Migration", () => {
    it("should migrate data between adapters", async () => {
      ...
      const thread = await source.createThread(createTestThread({ title: "Migration Test" }));
      await source.createMessage(createTestMessage(thread.id, { content: "Test message" }));
  ...
  describe("Factory Adapter Switching", () => {
    it("should switch between adapters using factory", async () => {
      ...
      const memory = await StorageFactory.createProvider("memory");
      await memory.init();
      expect(memory.type).toBe("memory");

      // Create another memory adapter (simulating switch)
      const memory2 = await StorageFactory.createProvider("mem");
      await memory2.init();
...
describe("High-Level Managers", () => {
  ...
  describe("ThreadManager", () => {
    ...
    it("should add messages to thread", async () => {
      ...
      const thread = await manager.createThread("user-1", "Thread with messages");
...
describe("Edge Cases and Special Scenarios", () => {
  ...
  describe("Special Characters", () => {
    ...
    it("should handle special characters in content", async () => {
      ...
      await storage.createMessage(createTestMessage(thread.id, { content: specialContent }));
  ...
  describe("Large Data", () => {
    it("should handle large content", async () => {
      ...
      await storage.createMessage(createTestMessage(thread.id, { content: largeContent }));
    ...
    it("should handle large arrays", async () => {
      const largeArray = Array.from({ length: 1000 }, (_, i) => ({ index: i, data: `item-${i}` }));
  ...
  describe("Concurrent Operations", () => {
    ...
    it("should handle concurrent message creation", async () => {
      ...
      const promises = Array.from({ length: 100 }, (_, i) =>
        storage.createMessage(createTestMessage(thread.id, { content: `Message ${i}` }))
      );
    ...
    it("should handle concurrent read and write", async () => {
      ...
      const operations = [
        storage.createMessage(createTestMessage(thread.id, { content: "Write 1" })),
        storage.getThread(thread.id),
        storage.createMessage(createTestMessage(thread.id, { content: "Write 2" })),
        storage.listThreads(),
        storage.createMessage(createTestMessage(thread.id, { content: "Write 3" })),
      ];
