...
describe("StorageRegistry", () => {
  ...
  describe("Backend Registration", () => {
    it("should register a storage backend", async () => {
      ...
      StorageRegistry.registerBackend("memory", {
        instance: storage,
        description: "In-memory storage",
      });

      const registered = await StorageRegistry.getBackend("memory");
    ...
    it("should register backend with factory function", async () => {
      ...
      const storage = await StorageRegistry.getBackend("memory");
    ...
    it("should register backend with aliases", async () => {
      ...
      const fromAlias1 = await StorageRegistry.getBackend("mem");
      const fromAlias2 = await StorageRegistry.getBackend("in-memory");
  ...
  describe("Backend Retrieval", () => {
    it("should return undefined for unregistered backend", async () => {
      const backend = await StorageRegistry.getBackend("nonexistent");
    ...
    it("should return registered backend instance", async () => {
      ...
      const retrieved = await StorageRegistry.getBackend("memory");
    ...
    it("should create backend from factory on first retrieval", async () => {
      ...
      await StorageRegistry.getBackend("memory");
  ...
  describe("Health Checks", () => {
    ...
    it("should check health of all backends", async () => {
      ...
      StorageRegistry.registerBackend("memory1", { instance: storage1 });
      StorageRegistry.registerBackend("memory2", { instance: storage2 });
  ...
  describe("Backend Listing", () => {
    it("should list all registered backends", async () => {
      ...
      StorageRegistry.registerBackend("memory1", {
        instance: storage1,
        description: "First memory storage",
      });
      StorageRegistry.registerBackend("memory2", {
        instance: storage2,
