...
await runSuite(async () => {
  ...
  await test("strips query strings and fragments from URL inputs", async () => {
    assertSnapshot(
      await detectExtension(
        "https://example.invalid/report.csv?download=true#preview",
      ),
      {
        type: "csv",
        mimeType: "text/csv",
  ...
  await test("strips query strings from non-URL extension candidates", async () => {
    assertSnapshot(await detectExtension("data.csv?download=true"), {
      type: "csv",
      mimeType: "text/csv",
      extension: "csv",
      source: "path",
  ...
  await test("keeps query and fragment characters in local filenames", async () => {
    assertSnapshot(await detectExtension("runs/exp#1.png"), {
      type: "image",
      mimeType: "image/png",
      extension: "png",
      source: "path",
    ...
    assertSnapshot(await detectExtension("snapshot?old.xlsx"), {
      type: "xlsx",
      mimeType:
  ...
  await test("falls back to path-like parsing for malformed URL inputs", async () => {
    ...
    assertSnapshot(await detectExtension("http://%%%"), {
      type: "unknown",
