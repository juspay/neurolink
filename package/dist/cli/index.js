...
function handleError(error, context) {
  ...
  if (error.message.includes("not authorized")) {
    console.error(
      chalk.yellow("💡 Check your account permissions for the selected model"),
    );
    console.error(
      chalk.yellow("💡 For AWS Bedrock: Use inference profile ARNs"),
    );
  }
  process.exit(1);
}
  ...
  .command(
    ...
    (yargs) =>
      yargs
        ...
        .example(
          '$0 generate-text "Technical doc" --format json',
          "Get JSON output",
        ),
    async (argv) => {
  ...
  .command(
    ...
    (yargs) =>
      yargs
        ...
        .example(
          '$0 stream "Explain AI" --provider vertex',
          "Stream with specific provider",
        ),
    async (argv) => {
      console.log(
        chalk.blue(`🔄 Streaming from ${argv.provider} provider...\n`),
      );
      try {
        if (!argv.prompt) {
          throw new Error("Prompt is required");
  ...
  .command(
    ...
    (yargs) =>
      yargs
        ...
        .example(
          "$0 batch prompts.txt --delay 2000",
    ...
    async (argv) => {
      try {
        ...
        for (let i = 0; i < prompts.length; i++) {
          const spinner = ora(
            `Processing ${i + 1}/${prompts.length}: ${prompts[i].substring(0, 50)}...`,
          ).start();
          try {
            const result = await sdk.generateText({
              prompt: prompts[i],
    ...
    async (argv) => {
      ...
      for (const provider of providers) {
        ...
        try {
          ...
          spinner.fail(
            `${provider}: ${chalk.red("❌ Failed")} - ${error.message}`,
          );
        }
      }
      // Show summary
