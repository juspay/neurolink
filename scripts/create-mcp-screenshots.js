...
const SCREENSHOTS_DIR = path.join(__dirname, '../docs/visual-content/screenshots/mcp-cli');
...
async function createCommandScreenshot(browser, command) {
  ...
  // Take screenshot
  const screenshotPath = path.join(SCREENSHOTS_DIR, `${command.name}-${new Date().toISOString().split('T')[0]}.png`);
...
${createdScreenshots.map(screenshot => `
### ${screenshot.title}
