...
const SCREENSHOTS_DIR = path.join(__dirname, '../docs/visual-content/screenshots/cli-screenshots');
...
🎯 Auto-selection: openai (fastest response)`
  },
  {
    ...
    command: 'npx @juspay/neurolink generate-text "Write a haiku about programming" --format json',
...
✅ Best provider: openai
  ...
  },
  {
    ...
    command: 'npx @juspay/neurolink batch prompts.txt --format json --output results.json',
...
async function createCommandScreenshot(browser, command) {
  ...
  const screenshotPath = path.join(SCREENSHOTS_DIR, `${command.name}-${timestamp}T12-00-00.png`);
