...
async function benchmarkProvider(providerName) {
  ...
  try {
    ...
    for (const promptConfig of BENCHMARK_PROMPTS) {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          ...
          promptResults.error = error.message;
    ...
    spinner.fail(
      `${chalk.red(providerName)} - Provider not available: ${error.message}`,
    );
  }
...
async function generateReport() {
  ...
  const availableProviders = Object.entries(benchmarkResults.providers).filter(
    ([_, results]) => results.available,
  );

  const providersBySpeed = availableProviders
    .filter(([_, results]) => results.avgResponseTime > 0)
  ...
  benchmarkResults.summary = {
    ...
    avgResponseTimeAcrossProviders: Math.round(
      providersBySpeed.reduce((sum, [_, r]) => sum + r.avgResponseTime, 0) /
