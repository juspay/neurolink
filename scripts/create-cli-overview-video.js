...
const OUTPUT_DIR = path.join(__dirname, '../docs/visual-content/videos/cli-videos/cli-overview');
...
async function createCLIOverviewVideo() {
...
🔧 github
  ...
  if (videoFile) {
    ...
    try {
      ...
      execSync(`ffmpeg -i "${newPath}" -c:v libx264 -preset medium -crf 23 -c:a aac -b:a 128k "${mp4Path}" -y`,
        { stdio: 'inherit' });
...
module.exports = { createCLIOverviewVideo };
