...
neurolink.on("toolConfirmationRequired", async (event) => {
  const { toolName, args, confirmationId } = event;
  ...
  if (approved) {
    neurolink.confirmToolExecution(confirmationId, true);
  } else {
    neurolink.confirmToolExecution(confirmationId, false);
  }
});
```
...
const neurolink = new NeuroLink({
  ...
  hitl: {
    enabled: true,
    auditLog: true, // Logs all approval decisions
  },
});
```
...
const factory = new MiddlewareFactory({
  middlewareConfig: {
    guardrails: {
      enabled: true,
      config: {
        badWords: ["harmful", "dangerous", "unsafe"],
        modelFilter: {
          enabled: true,
        },
        precallEvaluation: {
