...
- The `gemini-3-pro-image-preview` model requires `location: "global"` configuration on Vertex AI
- Other models can use regional endpoints like `us-east5` on Vertex AI
...
- `--imageOutput <path>`: Custom path for generated image (default: `generated-images/image-<timestamp>.png`)
- `--provider vertex` or `--provider google-ai`: Both Vertex AI and Google AI Studio support image generation
- `--model <model-name>`: Image generation model to use
...
- [Provider Status Monitoring](./PROVIDER-STATUS-MONITORING.md)
