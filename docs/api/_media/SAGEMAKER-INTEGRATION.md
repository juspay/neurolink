...
// Configure endpoint via environment variable
// Set SAGEMAKER_DEFAULT_ENDPOINT or SAGEMAKER_ENDPOINT_NAME before creating provider
process.env.SAGEMAKER_DEFAULT_ENDPOINT = "my-custom-model-endpoint";
process.env.AWS_REGION = "us-east-1";
process.env.AWS_ACCESS_KEY_ID = "your-access-key";
process.env.AWS_SECRET_ACCESS_KEY = "your-secret-key";

// Create SageMaker provider (uses endpoint from environment)
const provider = createBestAIProvider("sagemaker");

// Generate with configured endpoint
const result = await provider.generate({
  input: { text: "Analyze customer feedback for sentiment and themes" },
});

// Use different endpoint - update environment variable
process.env.SAGEMAKER_DEFAULT_ENDPOINT = "domain-expert-model-endpoint";

// Create new provider instance for the different endpoint
const domainProvider = createBestAIProvider("sagemaker");
...
class SecureSageMakerProvider {
  ...
  async secureGenerate(
    ...
    // Configure SageMaker endpoint via environment variable
    process.env.SAGEMAKER_DEFAULT_ENDPOINT = endpointName;
    process.env.AWS_REGION = this.region;

    // Create provider (endpoint comes from environment)
    const { createBestAIProvider } = await import("@juspay/neurolink");
    const provider = createBestAIProvider("sagemaker");
...
// Usage - Configure AWS credentials first
process.env.AWS_ACCESS_KEY_ID = "your-access-key";
process.env.AWS_SECRET_ACCESS_KEY = "your-secret-key";
...
const secureResult = await secureProvider.secureGenerate(
  ...
  "hipaa-compliant-model", // Endpoint name passed to method
  {
