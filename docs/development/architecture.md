...
interface ProviderRouter {
  ...
  // Failover handling
  handleFailover(failedProvider: Provider, request: GenerationRequest): Promise<Provider>;
}
...
class CloudScalabilityManager implements ScalabilityManager {
  ...
  async distributeLoad(requests: GenerationRequest[]): Promise<ProviderAssignment[]> {
  ...
  async checkSystemHealth(): Promise<HealthStatus> {
    ...
    const results = await Promise.allSettled(
      checks.map(([name, check]) => 
        this.executeHealthCheck(name, check)
      )
    );
