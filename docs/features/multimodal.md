...
`--transcribe-audio` is accepted but not yet implemented; it logs a warning
rather than silently producing nothing. Embedded **subtitle tracks** are
extracted and included today; spoken audio is transcribed only in the sense
that a native provider hears it directly.
...
| `--transcribe-audio` produced no transcript         | Not implemented yet                                                     | Use a native provider, or attach the audio separately                  |
