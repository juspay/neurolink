...
import { AuthProviderError, BaseAuthProvider } from "./BaseAuthProvider.js";
...
export class KeycloakProvider extends BaseAuthProvider {
  ...
  constructor(config: AuthProviderConfig) {
    ...
    if (!this.keycloakConfig.serverUrl) {
      throw AuthProviderError.create("CONFIGURATION_ERROR", "Keycloak serverUrl is required");
    }

    if (!this.keycloakConfig.realm) {
      throw AuthProviderError.create("CONFIGURATION_ERROR", "Keycloak realm is required");
    }

    if (!this.keycloakConfig.clientId) {
      throw AuthProviderError.create("CONFIGURATION_ERROR", "Keycloak clientId is required");
    }
    ...
    this.jwksCacheDuration = config.tokenValidation?.jwksCacheDuration ?? 600000; // 10 minutes
  ...
  async authenticateToken(token: string): Promise<TokenValidationResult> {
    try {
      ...
      if (this.keycloakConfig.verifyToken !== false && this.config.tokenValidation?.validateSignature !== false) {
      ...
      const validClaims: Record<string, import("../../types/common.js").JsonValue> = {};
  ...
  private async verifySignature(token: string): Promise<boolean> {
    try {
      ...
      // For full verification, use jose library
  ...
  private extractKeycloakUser(claims: TokenClaims): AuthUser {
    ...
    if (resourceAccess) {
      ...
      if (clientRoles) {
        roles = [...roles, ...clientRoles.map((r) => `${this.keycloakConfig.clientId}:${r}`)];
      }
    ...
    const providerData: Record<string, import("../../types/common.js").JsonValue> = {
      provider: "keycloak",
    };
    ...
    if (realmAccess !== undefined) {
      providerData.realm_access = realmAccess as import("../../types/common.js").JsonValue;
    }
    if (resourceAccess !== undefined) {
      providerData.resource_access = resourceAccess as import("../../types/common.js").JsonValue;
    }
  ...
  async getUser(userId: string): Promise<AuthUser | null> {
    ...
    try {
      ...
      for (const [key, value] of Object.entries(userData)) {
        if (value !== undefined) {
          providerData[key] = value as import("../../types/common.js").JsonValue;
        }
      }
      ...
      return {
        ...
        name:
          `${userData.firstName ?? ""} ${userData.lastName ?? ""}`.trim() || (userData.username as string | undefined),
        picture: undefined, // Keycloak doesn't store picture by default
        roles,
        permissions: [],
        emailVerified: userData.emailVerified as boolean | undefined,
        providerData,
        createdAt: userData.createdTimestamp ? new Date(userData.createdTimestamp as number) : undefined,
      };
    } catch (error) {
