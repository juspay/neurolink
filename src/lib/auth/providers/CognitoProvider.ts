...
export class CognitoProvider extends BaseAuthProvider {
  ...
  constructor(config: AuthProviderConfig) {
    ...
    if (config.type !== "cognito") {
      throw AuthProviderError.create("CONFIGURATION_ERROR", `Invalid provider type: ${config.type}. Expected: cognito`);
    }

    this.cognitoConfig = config as CognitoConfig;

    if (!this.cognitoConfig.userPoolId) {
      throw AuthProviderError.create("CONFIGURATION_ERROR", "Cognito userPoolId is required");
    }

    if (!this.cognitoConfig.clientId) {
      throw AuthProviderError.create("CONFIGURATION_ERROR", "Cognito clientId is required");
    }

    if (!this.cognitoConfig.region) {
      throw AuthProviderError.create("CONFIGURATION_ERROR", "Cognito region is required");
    }
  ...
  private extractCognitoUser(claims: TokenClaims, tokenUse: string): AuthUser {
    ...
    const email = claims.email ?? (claims["custom:email"] as string | undefined);
    ...
    const picture = claims.picture ?? (claims["custom:picture"] as string | undefined);
  ...
  async getUser(userId: string): Promise<AuthUser | null> {
    // Would require AWS SDK (aws-sdk or @aws-sdk/client-cognito-identity-provider)
    logger.debug("[CognitoProvider] getUser requires AWS SDK");
    return null;
  }
}
