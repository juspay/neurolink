...
import type { AuthProviderRegistration } from "./types/authTypes.js";
 ...
 */
export const AuthFactoryError = createErrorFactory("AuthFactory", AuthFactoryErrorCodes);
 ...
 * ```
 */
export class AuthProviderFactory extends BaseFactory<MastraAuthProvider, AuthProviderConfig> {
  ...
  protected async registerAll(): Promise<void> {
    ...
    this.registerProvider(
      ...
      async (config) => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return new CognitoProvider(config as any) as unknown as MastraAuthProvider;
      },
    ...
    this.registerProvider(
      ...
      async (config) => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return new KeycloakProvider(config as any) as unknown as MastraAuthProvider;
      },
      ["keycloak-oidc"],
      {
        name: "Keycloak",
  ...
  registerProvider(
    ...
    const registration: AuthFactoryRegistration = {
      factory: factory as (config: AuthProviderConfig) => Promise<MastraAuthProvider>,
      aliases,
      metadata,
    };
    ...
    logger.debug(`[AuthProviderFactory] Registered provider: ${type} with aliases: ${aliases.join(", ")}`);
  }
   ...
   */
  async create(typeOrAlias: string, config?: AuthProviderConfig): Promise<MastraAuthProvider> {
    ...
    if (!config) {
      throw AuthFactoryError.create("INVALID_CONFIG", `Configuration is required for provider: ${typeOrAlias}`, {
   ...
   */
  getProviderMetadata(type: string): AuthFactoryRegistration["metadata"] | undefined {
    const registration = this.registrations.get(type.toLowerCase());
