...
const PROVIDER_KEY_MAP: ProviderKeyMap = {
  ...
  [AIProviderName.AZURE]: ["azureOpenaiApiKey"],
  ...
  [AIProviderName.LITELLM]: ["litellmApiKey"],
  [AIProviderName.SAGEMAKER]: ["awsAccessKeyId", "awsSecretAccessKey"],
  [AIProviderName.OPENAI_COMPATIBLE]: ["openaiCompatibleApiKey"],
};
...
export function maskSecrets(inputs: ActionInputs): void {
  ...
  secrets.filter(Boolean).forEach((secret) => core.setSecret(secret!));
}
  ...
  return {
    ...
    thinking: {
      ...
      level: (core.getInput("thinking_level") || "medium") as
        ...
        | "medium"
        | "high",
      budget: parseInt(core.getInput("thinking_budget") || "10000", 10),
    },
