...
/**
 * Realtime Handler interface for provider-specific implementations
 *
 * Each provider (OpenAI Realtime, Gemini Live, etc.) implements this interface
 * to provide bidirectional voice communication.
 *
 * @example
 * ```typescript
 * class MyRealtimeHandler implements RealtimeHandler {
 *   async connect(config: RealtimeConfig): Promise<RealtimeSession> {
 *     // Establish WebSocket connection
 *   }
 *
 *   isConfigured(): boolean {
 *     return !!process.env.MY_REALTIME_API_KEY;
 *   }
 * }
 * ```
 */
export interface RealtimeHandler {
  ...
  /**
   * Connect to realtime service and create a session
   *
   * @param config - Session configuration
   * @returns Session information
   * @throws {RealtimeError} On connection failure
   */
   ...
   *
   * @throws {RealtimeError} On disconnection failure
   */
   ...
   *
   * @param audio - Audio chunk to send
   */
  sendAudio(audio: Buffer | RealtimeAudioChunk): Promise<void>;

  /**
   * Send text to the session (for text-based interactions)
   *
   ...
   * Used when turn detection is set to manual
   */
 ...
 * ```typescript
 * // Register a handler
 * RealtimeProcessor.registerHandler('openai', openaiHandler);
 *
 * // Connect to a session
 * const session = await RealtimeProcessor.connect('openai', {
 *   provider: 'openai',
 *   voice: 'alloy',
 ...
 *
 * // Send audio
 * await RealtimeProcessor.sendAudio('openai', audioBuffer);
 *
 * // Disconnect
 * await RealtimeProcessor.disconnect('openai');
 * ```
 */
