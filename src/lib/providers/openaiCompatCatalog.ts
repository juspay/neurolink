 ...
 * hand-written OpenAIChatCompletionsProvider subclass — see
 * ConfiguredOpenAICompatProvider for the class that reads these entries,
 * and providerRegistry.ts for the registration loop that consumes this
 * array.
 *
 ...
 * authoring format.
 *
 * To add a new OpenAI-compatible provider: add one JSON file under
 * src/lib/providers/catalog/ (see provider-catalog.schema.json) and run
 * `pnpm run codegen:catalog`. Do NOT add a provider here if it needs a real
 * hook override beyond the 3 mandatory ones
 * (getProviderName/getDefaultModel/formatProviderError) and the named
 * catalog quirks above — write a dedicated subclass instead (see
 * azureOpenai.ts).
 */
