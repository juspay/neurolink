 ...
 */
export type AgentEvaluationCriteria = {
  /** Check for accuracy */
  accuracy?: boolean;
 ...
 * Evaluation score
 */
export type AgentEvaluationScore = {
  /** Overall score (0-1) */
  overall: number;
...
export type OptimizationResult = {
  ...
  finalScore: AgentEvaluationScore;
 ...
 */
export type AgentEvaluatorInterface = {
  /** Evaluate a result */
  evaluate(
    result: AgentResult,
...
export type EvaluatedNetworkResult = NetworkExecutionResult & {
  /** Evaluation score */
  evaluationScore: AgentEvaluationScore;
 ...
 */
export type HandshakeRequest = ProtocolPayload & {
  protocol: "handshake";
  data: {
    agentId: string;
    agentName: string;
 ...
 */
export type HandshakeResponse = ProtocolPayload & {
  protocol: "handshake";
  data: {
    accepted: boolean;
    agentId: string;
 ...
 */
export type DiscoveryRequest = ProtocolPayload & {
  protocol: "discovery";
  data: {
 ...
 */
export type DiscoveryResponse = ProtocolPayload & {
  protocol: "discovery";
  data: {
    agents: Array<{
      id: string;
 ...
 */
export type DelegationRequest = ProtocolPayload & {
  protocol: "delegation";
  data: {
    task: string;
    context?: Record<string, unknown>;
 ...
 */
export type DelegationResponse = ProtocolPayload & {
  protocol: "delegation";
  data: {
    accepted: boolean;
    result?: AgentResult;
 ...
 */
export type AggregationRequest = ProtocolPayload & {
 ...
 */
export type ConsensusRequest = ProtocolPayload & {
  protocol: "consensus";
  data: {
    proposal: unknown;
    participants: string[];
 ...
 */
export type ConsensusVote = ProtocolPayload & {
  protocol: "consensus";
  state: "active";
  data: {
 ...
 * Heartbeat payload
 */
export type HeartbeatPayload = ProtocolPayload & {
  protocol: "heartbeat";
  data: {
    agentId: string;
...
// ============================================================================
// From src/lib/agent/supervisor/supervisor-agent.ts
// ============================================================================
