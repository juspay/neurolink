...
function hasRestrictedOutputLimit(model?: string): boolean {
  ...
  if (model.includes("gemini-3")) return true;

  // Check for image generation models (includes gemini-2.5-flash-image)
  if (IMAGE_GENERATION_MODELS.some(m => model.includes(m))) return true;
...
export function getSafeMaxTokens(
  ...
  if (isRestrictedModel) {
    // If requested exceeds limit, cap it
    if (requestedMaxTokens && requestedMaxTokens > GEMINI_RESTRICTED_MAX_OUTPUT_TOKENS) {
      logger.warn(
    ...
    // Otherwise, use the requested value (it's within limits)
