...
export class TTSProcessor {
  ...
  static async getVoices(
    ...
    // Same two guards synthesize() enforces, so getVoices() keeps the
    // `@throws TTSError` contract above regardless of what a given handler's
    // own implementation does: block on configuration before the handler
    // runs at all, and shape whatever it throws through the same
    // toSynthesisError() synthesize() uses, so `code`/`retriable`/category
    // survive for a caller keying retry logic off them.
    if (!handler.isConfigured()) {
      throw new TTSError({
        code: TTS_ERROR_CODES.PROVIDER_NOT_CONFIGURED,
        message: `TTS provider "${providerName}" is not configured. Please set the required API keys.`,
    ...
    try {
      ...
      // `getVoices`'s own options ({ languageCode }) share no fields with
      // `TTSOptions` — synthesize()'s options shape — so there is nothing
      // meaningful to forward into toSynthesisError()'s context here.
      throw this.toSynthesisError(error, providerName, "", {});
    }
  }
