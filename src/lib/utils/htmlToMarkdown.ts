...
function parseHtml(html: string): MarkdownHtmlNode[] {
  ...
  while ((match = tagPattern.exec(html)) !== null) {
    ...
    if (isClosing) {
      // Only unwind if the tag is actually open, so a stray `</div>` is
      // inert. Search from the top of the stack down so a closing tag
      // matches the innermost (most recently opened) element with that
      // name — `findIndex` would match the OUTERMOST one instead, and for
      // nested same-tag elements (`<div><div>A</div>B</div>`) that pops both
      // levels at once, reattaching everything after the real inner close to
      // the wrong ancestor.
      let openIndex = -1;
...
/**
 * Format a link/image destination per CommonMark's angle-bracket form.
 *
 * A bare `(dest)` destination ends at the first whitespace or unbalanced
 * `)`, so a value containing either — an `href`/`src` with a space, or one
 * with unbalanced parentheses — would silently truncate or corrupt the
 * surrounding `[text](dest)` / `![alt](dest)` markup. Wrapping in `<...>`
 * makes the destination unambiguous; CommonMark requires any literal `<`
 * or `>` inside that form to be escaped. Ordinary destinations (no
 * whitespace or parens) are returned byte-identical to before.
 */
function formatLinkDestination(dest: string): string {
  if (!/[\s()]/.test(dest)) {
    return dest;
  }
  // Backslash first: escaping only `<`/`>` turns `\>` into `\\>`, which
  // CommonMark reads as a literal backslash followed by a bare `>`.
  const escaped = dest
    .replace(/\\/g, "\\\\")
    .replace(/</g, "\\<")
    .replace(/>/g, "\\>");
  ...
  for (const node of nodes) {
    ...
    switch (node.tag) {
      ...
      case "a": {
        ...
        out += href ? `[${inner}](${formatLinkDestination(href)})` : inner;
        break;
      }
      case "img": {
...
/**
 * Extract a `<pre>` block's text content verbatim.
 *
 * `renderInline()` funnels every text node through `collapseWhitespace()`,
 * which does `.replace(/\s+/g, " ")` — right for prose, destructive for
 * code: it collapses every newline, tab and indentation run onto one line.
 * This walks the same tree but keeps each text node's raw value, so a
 * multi-line, indented code sample survives into the fenced block intact.
 * A nested element (e.g. `<pre><code>…</code></pre>`) still contributes its
 * text, but ignores inline markdown semantics like `**`/`*`/`` ` `` —
 * `<pre>` content is literal code, not prose, and injecting markdown markers
 * into it would corrupt the code rather than format it.
 */
function renderPreformattedText(nodes: MarkdownHtmlNode[]): string {
  let out = "";
  for (const node of nodes) {
    if (node.kind === "text") {
      out += node.value;
    ...
    out += renderPreformattedText(node.children);
  }
  return out;
}
...
function renderBlocks(nodes: MarkdownHtmlNode[]): string {
  ...
  for (const node of nodes) {
    ...
    switch (node.tag) {
      ...
      case "pre": {
        flushInline();
        const inner = renderPreformattedText(node.children).replace(
          /^\n+|\n+$/g,
          "",
        );
        if (inner) {
