...
export class AgentEvaluator implements AgentEvaluatorInterface {
   ...
   */
  private buildCriteriaDescription(criteria: EvaluationCriteria): string {
    const parts: string[] = [];
   ...
   */
  private parseEvaluationResponse(response: string): EvaluationScore | null {
    try {
  ...
  private fallbackEvaluation(
    ...
    let overall = 0.5;
    const breakdown: EvaluationScore["breakdown"] = {};
   ...
   */
  private fallbackSuggestions(score: EvaluationScore): ImprovementSuggestion[] {
...
${suggestionText}`;
  ...
  async compareResults(
    ...
    task: string,
  ): Promise<Array<{ agentId: string; score: EvaluationScore; rank: number }>> {
    const criteria: EvaluationCriteria = {
      accuracy: true,
      completeness: true,
      relevance: true,
