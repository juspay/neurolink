 ...
 */
export type CoordinationContext = ExecutionContext & {
...
export class AgentCoordinator {
  ...
  async coordinate(
    ...
    try {
      ...
      switch (config.strategy) {
        case "sequential":
          result = await this.executeSequential(task, context);
          break;
        case "parallel":
          result = await this.executeParallel(task, context);
          break;
        case "pipeline":
          result = await this.executePipeline(task, context);
          break;
        case "roundRobin":
          result = await this.executeRoundRobin(task, context);
          break;
        case "leastBusy":
          result = await this.executeLeastBusy(task, context);
          break;
        case "custom":
  ...
  private async executePipeline(
    ...
    return this.executeSequential(task, context);
  }
  ...
  private async executeRoundRobin(
    ...
    // For round robin, select the next agent in rotation
    const selectedIndex = context.currentStep % agents.length;
