...
export function setupWebSocket(server: HttpServer) {
  ...
  wss.on("connection", (clientWs) => {
    void (async () => {
      ...
      let cobra: CobraInstance | null = null;
      let FRAME_LENGTH = 512;
      let FRAME_BYTES = FRAME_LENGTH * 2;
      try {
        let mod: { Cobra: new (key: string) => CobraInstance };
        try {
          mod = (await import(
          ...
          const e =
            err instanceof Error ? (err as NodeJS.ErrnoException) : null;
          if (
      ...
      async function processTurn(userText: string) {
        ...
        try {
          ...
          if (ttsError) {
            ...
            closeTts(
              tts,
              "[TTS] Failed to close stream after mid-stream error",
            );
            turnManager.reset();
            return;
          }
          ...
          try {
            await withTimeout(
              new Promise<void>((resolve, reject) => {
                ...
                tts.once("close", () =>
                  reject(
                    new Error("Cartesia WS closed before flush completed"),
                  ),
                );
                tts.flush();
              }),
          ...
          closeTts(
            tts,
            "[TTS] Failed to close stream after successful playback",
          );
