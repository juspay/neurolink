 ...
 *
 * Google accepts three aliases because `.env.example` documents
 * `GOOGLE_AI_API_KEY`/`GEMINI_API_KEY` as the canonical Google credentials —
 * the same aliasing `GoogleSTT` itself does, kept in sync here so
 * availability cannot disagree with what the handler will accept.
 */
...
> = {
  ...
  google: {
    label: "google-stt",
    envVars: ["GOOGLE_API_KEY", "GOOGLE_AI_API_KEY", "GEMINI_API_KEY"],
  },
...
export class AudioProcessor extends BaseFileProcessor<ProcessedAudio> {
  ...
  private async attemptTranscription(
     ...
     */
    const skipped = (reason: string): AudioTranscriptionOutcome => {
      logger.warn(
  ...
  private async transcribeWithOpenAI(
    ...
    skipped: (reason: string) => AudioTranscriptionOutcome,
  ): Promise<AudioTranscriptionOutcome> {
  ...
  private async transcribeWithHandler(
    ...
    try {
      ...
      if (text.length === 0) {
        return skipped(`${label} returned an empty transcript for this audio`);
      }
