...
/**
 * Ceiling for natively-delivered video in one request, in MB of *source*
 * bytes.
 *
 * Gemini's documented limit is 20 MB for the whole request. Inline data is
 * base64 in a JSON body, which costs 4 bytes per 3, so a 20 MB source file
 * arrives as ~26.7 MB of request and is rejected.
 *
 * The 20 MB limit is request-wide, not per-part: a video alone consumes it
 * fully once its encoded size reaches 20 MB, leaving nothing for the prompt,
 * system instructions, tool schemas or the JSON structure carrying all of
 * them in the same request. Dividing the raw budget by the 4/3 inflation —
 * 20 ÷ 4/3 = 15 MB — finds the largest source size whose *video part alone*
 * still fits, but that calculation has no margin in it: 15 × 4/3 = 20.0 MB
 * exactly, so any non-empty prompt sharing that request pushes the total
 * over the documented limit and the request is rejected upstream, for a
 * reason this module never sees. That was the source of the previous value
 * here — it satisfied "fits under 20 MB" only for a request containing
 * nothing else, which no real request is.
 *
 * 14 MB is used instead: it still encodes to well under the 20 MB limit
 * (14 × 4/3 ≈ 18.7 MB), leaving roughly 1.3 MB of headroom in the same
 * request for the prompt and everything else sharing it.
 *
 * This is a request-wide budget, not a per-clip one: two clips at 12 MB each
 * pass this check individually but together would arrive as ~32 MB of
 * request. `canDeliverVideoNatively`'s optional `priorNativeVideoBytes`
 * parameter lets a caller iterating multiple clips in one request charge
 * each clip against what earlier clips already committed, so the ceiling is
 * enforced across the whole request rather than reset per clip.
 *
 ...
 */
const GEMINI_INLINE_VIDEO_MAX_MB = 14;
...
/**
 * Duration assumed for a natively-delivered clip whose length is unknown.
 *
 * `canDeliverVideoNatively` treats an unknown duration as no reason to
 * refuse (probing fails on exotic containers and on machines without
 * ffmpeg — exactly the clips most likely to reach this branch), so a clip
 * priced here with `durationSec: 0` can still be the one that just travelled
 * to Gemini as bytes. Pricing that as 0 tokens would make a real payload
 * look free. 5s is a deliberately small floor: enough that no natively
 * delivered clip is ever budgeted as free, without pretending to know how
 * long an unmeasured one actually runs.
 */
const MIN_NATIVE_VIDEO_ESTIMATE_SEC = 5;
...
/** Vertex aliases GoogleVertexProvider serves — the only front end that can
 * route a single request to either Gemini or Claude depending on model id. */
const VERTEX_ALIASES: ReadonlySet<string> = new Set([
  "vertex",
  "google-vertex",
  "googlevertex",
]);
...
/**
 * Whether `model` is one `GoogleVertexProvider.isAnthropicModel` (client.ts)
 * would route to the native Anthropic Vertex SDK rather than Gemini.
 *
 * Duplicated here rather than imported: `isAnthropicModel` lives in
 * `providers/googleVertex/client.ts`, and importing a provider module from an
 * adapter would put `adapters/` on the wrong side of the provider/adapter
 * dependency direction. `providerImageAdapter.ts` carries the same
 * `model.includes("claude")` check for the same reason — keep both in sync
 * if Vertex's routing predicate ever changes.
 */
...
export function getVideoProviderConfig(
  ...
  if (VERTEX_ALIASES.has(normalized) && model && isClaudeModelId(model)) {
    return VIDEO_PROVIDER_CONFIGS.anthropic;
  }
  return VIDEO_PROVIDER_CONFIGS[normalized] ?? null;
}

/** Whether `provider` (optionally, running `model`) can be handed raw video
 * bytes. See {@link getVideoProviderConfig} for the Claude-on-Vertex case. */
export function supportsNativeVideo(provider: string, model?: string): boolean {
  return getVideoProviderConfig(provider, model)?.supportsNativeVideo ?? false;
}
...
export function estimateVideoTokens(options: {
  ...
  if (config?.supportsNativeVideo) {
    // seconds === 0 covers both "genuinely 0s" and "unmeasured" (the
    // documented meaning of durationSec: 0 above) — either way a clip that
    // reaches this branch is being delivered natively, so it is never free.
    const nativeSeconds = seconds > 0 ? seconds : MIN_NATIVE_VIDEO_ESTIMATE_SEC;
    return Math.ceil(nativeSeconds * TOKENS_PER_NATIVE_VIDEO_SECOND);
  }
 ...
 *
 * @param priorNativeVideoBytes - Source bytes already committed to the
 *   *same request's* inline budget: every inline image (keyframes
 ...
 *   per-clip check.
 * @param model - See {@link getVideoProviderConfig}: disambiguates a
...
export function canDeliverVideoNatively(
  ...
  priorNativeVideoBytes: number = 0,
  model?: string,
): VideoDeliveryDecision {
  const config = getVideoProviderConfig(provider, model);
  if (!config?.supportsNativeVideo) {
    return {
      deliver: false,
  ...
  // Charged cumulatively: this clip's bytes plus everything the same request
  // has already committed to the inline budget (other inline parts and
  // earlier clips). With the default of 0 this is a per-clip-only check.
  const aggregateBytes =
    Math.max(0, priorNativeVideoBytes) + video.buffer.length;
  const sizeMB = video.buffer.length / (1024 * 1024);
  const aggregateMB = aggregateBytes / (1024 * 1024);
  if (aggregateMB > config.maxSizeMB) {
    const reason =
      priorNativeVideoBytes > 0
        ? `the clip is ${sizeMB.toFixed(1)}MB, which would bring this ` +
          `request's inline attachments to ${aggregateMB.toFixed(1)}MB, ` +
          `over the ${config.maxSizeMB}MB inline ceiling shared by the whole ` +
