...
import type { StorageMiddleware, CachingMiddlewareConfig } from "../types/storageTypes.js";
...
export class CachingMiddleware implements StorageMiddleware {
  ...
  invalidatePattern(pattern: RegExp): number {
    ...
    if (count > 0) {
      logger.debug(`CachingMiddleware: Invalidated ${count} entries matching pattern`);
    }
    return count;
  }
