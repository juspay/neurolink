...
import { gzipSync, gunzipSync, deflateSync, inflateSync, brotliCompressSync, brotliDecompressSync, constants } from "zlib";
import { logger } from "../../utils/logger.js";
import { createErrorFactory } from "../../core/infrastructure/baseError.js";
import type { StorageMiddleware, CompressionMiddlewareConfig } from "../types/storageTypes.js";
...
type SupportedAlgorithm = "gzip" | "deflate" | "brotli" | "lz4";
...
export class CompressionMiddleware implements StorageMiddleware {
  ...
  constructor(config: CompressionMiddlewareConfig = {}) {
    this.algorithm = (config.algorithm as SupportedAlgorithm) || "gzip";
    ...
    if (this.algorithm === "lz4") {
      logger.warn("CompressionMiddleware: lz4 not available, falling back to gzip");
      this.algorithm = "gzip";
    }
  }
  ...
  async init(): Promise<void> {
    ...
    logger.debug(`CompressionMiddleware: Initialized with ${this.algorithm} level ${this.level}`);
  }
  ...
  async beforeWrite(key: string, value: JsonValue): Promise<JsonValue> {
    if (!this.initialized) {
      throw CompressionErrors.create("COMPRESSION_FAILED", "Middleware not initialized");
    }
  ...
  async afterRead(key: string, value: JsonValue): Promise<JsonValue> {
    if (!this.initialized) {
      throw CompressionErrors.create("DECOMPRESSION_FAILED", "Middleware not initialized");
    }

    // Check if data is compressed
    if (!this.isCompressedPayload(value)) {
      return value;
    }

    try {
      const payload = value as unknown as CompressedPayload;
   ...
   * Decompress data
   */
  private decompress(data: Buffer, algorithm: SupportedAlgorithm): Buffer {
    switch (algorithm) {
      case "gzip":
   ...
   */
  private isCompressedPayload(value: unknown): value is CompressedPayload {
    return (
      typeof value === "object" &&
      value !== null &&
