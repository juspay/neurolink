...
import type { StorageMiddleware, EncryptionMiddlewareConfig } from "../types/storageTypes.js";
...
type SupportedAlgorithm = "aes-256-gcm" | "aes-256-cbc" | "chacha20-poly1305";
type SupportedKdf = "pbkdf2" | "scrypt";
...
export class EncryptionMiddleware implements StorageMiddleware {
  ...
  async beforeWrite(key: string, value: JsonValue): Promise<JsonValue> {
    if (!this.initialized) {
      throw EncryptionErrors.create("ENCRYPTION_FAILED", "Middleware not initialized");
    }
  ...
  async afterRead(key: string, value: JsonValue): Promise<JsonValue> {
    if (!this.initialized) {
      throw EncryptionErrors.create("DECRYPTION_FAILED", "Middleware not initialized");
    }
    ...
    try {
      const decrypted = this.decrypt(value as unknown as EncryptedPayload);
   ...
   */
  private deriveKey(keyInput: string, kdf?: SupportedKdf, salt?: string): Buffer {
