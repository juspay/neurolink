 ...
 * StorageRegistry.registerBackend('custom', {
 ...
 * const storage = await StorageRegistry.getBackend('custom');
 *
 * // Check health
...
export class StorageRegistry {
   ...
   * Factory is provided separately through StorageFactory
   */
    ...
    this.entries.set(type, {
      factory: async () => {
        throw RegistryErrors.create(
          "NOT_FOUND",
          `Backend '${type}' has no factory registered. Use StorageFactory.createStorage() instead.`,
        );
      },
      metadata,
  ...
  /**
   * Register a storage backend with factory or instance
   */
  static registerBackend(
    ...
    registry.events.emit("backend:registered", type as StorageBackendType, metadata);
    logger.debug(`StorageRegistry: Registered backend '${type}'`);
  }
  ...
  static unregisterBackend(type: StorageBackendType | string): boolean {
    ...
    logger.debug(`StorageRegistry: Unregistered backend '${type}'`);

    return true;
  }

  /**
   * Get backend instance (creates if not exists)
   */
  static async getBackend(
    ...
    try {
      ...
      logger.debug(`StorageRegistry: Created instance for '${resolvedType}'`);
      return instance;
    } catch (error) {
      throw RegistryErrors.create(
   ...
   */
  static async checkAllHealth(): Promise<Map<StorageBackendType, HealthCheckResult>> {
    const registry = StorageRegistry.getInstance();
    await registry.ensureInitialized();
   ...
   */
  static async getStats(): Promise<RegistryStats> {
    const registry = StorageRegistry.getInstance();
    await registry.ensureInitialized();
...
export const registerBackend = StorageRegistry.registerBackend;
export const unregisterBackend = StorageRegistry.unregisterBackend;
export const getBackend = StorageRegistry.getBackend;
