 ...
 */
export class S3StorageAdapter implements MastraStorage {
  ...
  async init(_options?: StorageInitOptions): Promise<void> {
    ...
    logger.debug("S3StorageAdapter: Initializing");
    ...
    try {
      ...
      this.client = new S3(clientConfig) as unknown as S3Client;
      ...
      logger.info("S3StorageAdapter: Initialized successfully");
    } catch (error) {
      throw S3Errors.create(
        "CONNECTION_FAILED",
  ...
  async close(): Promise<void> {
    ...
    logger.info("S3StorageAdapter: Closed");
  }
  ...
  async deleteThread(threadId: string): Promise<boolean> {
    this.ensureInitialized();
    if (!this.threadIndex.has(threadId)) return false;
  ...
  async createMessage(
    ...
    await this.putObject(`messages/${message.threadId}/${newMessage.id}.json`, newMessage);
  ...
  async updateMessage(
    ...
    await this.putObject(`messages/${existing.threadId}/${messageId}.json`, updated);
  ...
  async deleteRecord(namespace: string, key: string): Promise<boolean> {
    this.ensureInitialized();
    if (!this.recordIndex.has(`${namespace}:${key}`)) return false;
  ...
  async getStats(): Promise<StorageStats> {
    ...
    return {
      ...
      messageCount: Array.from(this.messageIndex.values()).reduce((acc, m) => acc + m.length, 0),
  ...
  async clearAll(): Promise<void> {
    ...
    logger.warn("S3StorageAdapter: All data cleared");
  }
  ...
  private async loadIndexes(): Promise<void> {
    logger.debug("S3StorageAdapter: Loading indexes");
    ...
    for (const obj of listResult.Contents) {
      ...
      try {
        ...
        logger.warn(`S3StorageAdapter: Failed to load object ${key}`);
      }
    }

    logger.debug(`S3StorageAdapter: Loaded ${this.threadIndex.size} threads, ${this.workflowIndex.size} workflows`);
  }
}
