...
export class SQLiteStorageAdapter implements MastraStorage {
  ...
  constructor(config: StorageConfig | Omit<SQLiteStorageConfig, "type">) {
    // Allow config without type field for convenience
    const configWithType = "type" in config ? config : { ...config, type: "sqlite" as const };
  ...
  async init(options?: StorageInitOptions): Promise<void> {
    ...
    logger.debug(`SQLiteStorageAdapter: Initializing with ${this.config.filename}`);

    try {
      // Try better-sqlite3 first, fall back to sql.js
      const Database = await this.loadDatabaseDriver();
      this.db = new Database(this.config.filename) as unknown as SQLiteDatabase;
  ...
  async getThread(threadId: string): Promise<StorageThread | null> {
    this.ensureInitialized();
    const row = this.db!.prepare(`SELECT * FROM ${this.table("threads")} WHERE id = ?`).get(threadId) as
  ...
  async updateThread(
    ...
    const result = this.db!.prepare(`UPDATE ${this.table("threads")} SET ${setClauses.join(", ")} WHERE id = ?`).run(
      ...params,
    );
  ...
  async deleteThread(threadId: string): Promise<boolean> {
    ...
    const result = this.db!.prepare(`DELETE FROM ${this.table("threads")} WHERE id = ?`).run(threadId);
  ...
  async listThreads(options?: ThreadQueryOptions): Promise<PaginatedResult<StorageThread>> {
    ...
    const countRow = this.db!.prepare(`SELECT COUNT(*) as count FROM ${this.table("threads")} ${whereClause}`).get(
      ...params,
    ) as { count: number };
  ...
  async getMessage(messageId: string): Promise<StorageMessage | null> {
    this.ensureInitialized();
    const row = this.db!.prepare(`SELECT * FROM ${this.table("messages")} WHERE id = ?`).get(messageId) as
      | Record<string, unknown>
      | undefined;
  ...
  async updateMessage(
    ...
    const result = this.db!.prepare(`UPDATE ${this.table("messages")} SET ${setClauses.join(", ")} WHERE id = ?`).run(
      ...params,
    );
  ...
  async deleteMessage(messageId: string): Promise<boolean> {
    this.ensureInitialized();
    const result = this.db!.prepare(`DELETE FROM ${this.table("messages")} WHERE id = ?`).run(messageId);
    return result.changes > 0;
  }
  ...
  async listMessages(options: MessageQueryOptions): Promise<PaginatedResult<StorageMessage>> {
    ...
    const countRow = this.db!.prepare(`SELECT COUNT(*) as count FROM ${this.table("messages")} ${whereClause}`).get(
      ...params,
    ) as { count: number };
    const total = countRow.count;
  ...
  async deleteMessagesByThreadId(threadId: string): Promise<number> {
    this.ensureInitialized();
    const result = this.db!.prepare(`DELETE FROM ${this.table("messages")} WHERE thread_id = ?`).run(threadId);
  ...
  async saveWorkflowRun(
    ...
    this.db!.prepare(
      ...
      workflowRun.output !== undefined ? JSON.stringify(workflowRun.output) : null,
      ...
      workflowRun.suspensionData ? JSON.stringify(workflowRun.suspensionData) : null,
  ...
  async getWorkflowRun(runId: string): Promise<StorageWorkflowRun | null> {
    this.ensureInitialized();
    const row = this.db!.prepare(`SELECT * FROM ${this.table("workflow_runs")} WHERE id = ?`).get(runId) as
      | Record<string, unknown>
      | undefined;
  ...
  async getRecord(namespace: string, key: string): Promise<StorageCustomRecord | null> {
    this.ensureInitialized();
    const row = this.db!.prepare(`SELECT * FROM ${this.table("records")} WHERE namespace = ? AND key = ?`).get(
      namespace,
      key,
    ...
    if (record.ttl) {
      const expiresAt = new Date(record.updatedAt.getTime() + record.ttl * 1000);
  ...
  async deleteRecord(namespace: string, key: string): Promise<boolean> {
    this.ensureInitialized();
    const result = this.db!.prepare(`DELETE FROM ${this.table("records")} WHERE namespace = ? AND key = ?`).run(
      namespace,
      key,
    );
    return result.changes > 0;
  ...
  async listRecords(namespace: string, options?: QueryOptions): Promise<PaginatedResult<StorageCustomRecord>> {
    ...
    const countRow = this.db!.prepare(`SELECT COUNT(*) as count FROM ${this.table("records")} WHERE namespace = ?`).get(
      namespace,
    ) as { count: number };
    const total = countRow.count;
  ...
  async deleteNamespace(namespace: string): Promise<number> {
    this.ensureInitialized();
    const result = this.db!.prepare(`DELETE FROM ${this.table("records")} WHERE namespace = ?`).run(namespace);
    return result.changes;
  }
  ...
  async getStats(): Promise<StorageStats> {
    ...
    const threadCount = (
      this.db!.prepare(`SELECT COUNT(*) as count FROM ${this.table("threads")}`).get() as { count: number }
    ).count;

    const messageCount = (
      this.db!.prepare(`SELECT COUNT(*) as count FROM ${this.table("messages")}`).get() as { count: number }
    ).count;

    const workflowRunCount = (
      this.db!.prepare(`SELECT COUNT(*) as count FROM ${this.table("workflow_runs")}`).get() as { count: number }
    ).count;

    const customRecordCount = (
      this.db!.prepare(`SELECT COUNT(*) as count FROM ${this.table("records")}`).get() as { count: number }
    ).count;
  ...
  private async loadDatabaseDriver(): Promise<new (filename: string) => unknown> {
    try {
      const { default: Database } = await import("better-sqlite3");
      return Database;
    } catch {
      // Fallback to sql.js or throw
  ...
  private mapWorkflowRun(row: Record<string, unknown>): StorageWorkflowRun {
    return {
      ...
      stepResults: row.step_results ? JSON.parse(row.step_results as string) : undefined,
      suspensionData: row.suspension_data ? JSON.parse(row.suspension_data as string) : undefined,
