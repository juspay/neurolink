...
import { ContextManager, createExecutionContext } from '../lib/mcp/context-manager.js';
...
describe('AI Analysis Tools Tests', () => {
  ...
  beforeEach(async () => {
    ...
    // Register AI Core Server with new tools
  ...
  describe('AI Usage Analysis Tool', () => {
    ...
    it('should analyze specific provider usage', async () => {
      ...
      expect(result.data.analysis.providers.openai.cost).toBeGreaterThanOrEqual(0);
  ...
  describe('Provider Performance Benchmarking Tool', () => {
    ...
    it('should benchmark specific providers with custom prompts', async () => {
      ...
      expect(result.data.testConfiguration.providers).toEqual(['openai', 'vertex']);
  ...
  describe('Prompt Parameter Optimization Tool', () => {
    it('should optimize parameters for balanced style', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.data.optimization.originalPrompt).toBe('Write a short story about AI');
      expect(result.data.optimization.style).toBe('balanced');
      expect(result.data.optimization.optimizeFor).toBe('quality');

      // Check optimization results
      expect(result.data.results).toHaveLength(3);
      expect(result.data.results[0].parameters.temperature).toBeGreaterThanOrEqual(0.5);
      expect(result.data.results[0].parameters.temperature).toBeLessThanOrEqual(0.8);
      expect(result.data.results[0].parameters.maxTokens).toBeGreaterThanOrEqual(100);
      expect(result.data.results[0].parameters.maxTokens).toBeLessThanOrEqual(300);

      // Check recommendations
      expect(result.data.recommendations.optimal).toBeDefined();
      expect(result.data.recommendations.optimal.temperature).toBeGreaterThan(0);
  ...
  describe('Integration Tests: AI Analysis Tools', () => {
    ...
    it('should verify tool schemas and metadata', () => {
      const tools = ['analyze-ai-usage', 'benchmark-provider-performance', 'optimize-prompt-parameters'];
    ...
    it('should validate Phase 1.1 performance requirements', async () => {
      ...
      const results = await Promise.all([
        ...
        registry.executeTool('benchmark-provider-performance', { iterations: 1 }, context),
        registry.executeTool('optimize-prompt-parameters', { prompt: 'Performance test', iterations: 2 }, context)
      ]);
      ...
      console.log(`✅ Phase 1.1 performance validation: ${totalExecutionTime}ms total execution time`);
    });
  });
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    it('should maintain backward compatibility', () => {
      ...
      const originalTools = ['generate-text', 'select-provider', 'check-provider-status'];
