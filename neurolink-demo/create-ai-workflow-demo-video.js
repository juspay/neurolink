  ...
  try {
    ...
    await page.evaluate(() => {
      const workflowSection = Array.from(document.querySelectorAll('*')).find(el =>
        el.textContent && el.textContent.includes('AI Development Workflow Tools')
      );
      if (workflowSection) {
    ...
    const testCasesButton = page.locator('button:has-text("Generate Test Cases")').first();
    ...
    const refactorButton = page.locator('button:has-text("Refactor Code")').first();
    if (await refactorButton.isVisible()) {
      await refactorButton.click();
      await page.waitForTimeout(DELAY_BETWEEN_ACTIONS);

      await page.screenshot({
        path: path.join(__dirname, 'screenshots', '11-refactor-code-result.png'),
        fullPage: false
      });
    }
    ...
    const docsButton = page.locator('button:has-text("Generate Documentation")').first();
    if (await docsButton.isVisible()) {
      await docsButton.click();
      await page.waitForTimeout(DELAY_BETWEEN_ACTIONS);

      await page.screenshot({
        path: path.join(__dirname, 'screenshots', '12-documentation-result.png'),
        fullPage: false
      });
    }
    ...
    const debugButton = page.locator('button:has-text("Debug AI Output")').first();
    ...
    console.log('✅ AI Workflow Tools Demo Video recording completed successfully!');
